...
import {
  ChevronRight, Film, Loader2, Maximize, Minimize,
  Pause, Play, Volume2, VolumeX, MessageCircle, Send,
  Pencil, Trash2, Reply, ChevronUp, Check, X
} from 'lucide-react'
...
const Respuestas = ({ videoId, commentId, User, isAdmin }) => {
  ...
  return (
    <div className="mt-3 pl-4 border-l-2 border-gray-100 space-y-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {(isAdmin || r.autorId === User?.uid) && editandoId !== r.id && (
                <div className="flex items-center gap-1 ml-auto">
                  {r.autorId === User?.uid && (
                    <button onClick={() => { setEditandoId(r.id); setTextoEdit(r.texto) }}
            ...
            {editandoId === r.id ? (
              <div className="flex gap-1.5 mt-1">
                <input value={textoEdit} onChange={e => setTextoEdit(e.target.value)}
                  ...
                  autoFocus />
                <button onClick={() => editar(r.id)} disabled={!textoEdit.trim() || editando}
      ...
      <form onSubmit={e => { e.preventDefault(); if (texto.trim()) enviar() }} className="flex gap-2 pt-1">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {(puedeEditar || puedeEliminar) && !editando && (
            <div className="flex items-center gap-1 ml-auto">
              {puedeEditar && (
                <button onClick={() => { setEditando(true); setTextoEdit(c.texto) }}
                  className="p-1 text-gray-400 hover:text-blue-500 transition-colors rounded" title="Editar">
                  <Pencil className="w-3.5 h-3.5" />
                </button>
              )}
              {puedeEliminar && (
                <button onClick={() => eliminar()}
                  className="p-1 text-gray-400 hover:text-red-500 transition-colors rounded" title="Eliminar">
        ...
        {editando ? (
          <div className="flex gap-1.5 mt-1">
            <input value={textoEdit} onChange={e => setTextoEdit(e.target.value)}
              ...
              autoFocus />
            <button onClick={() => editar()} disabled={!textoEdit.trim() || guardando}
        ...
        {!editando && (
          <button onClick={() => setMostrarRespuestas(r => !r)}
        ...
        {mostrarRespuestas && (
          <Respuestas videoId={videoId} commentId={c.id} User={User} isAdmin={isAdmin} />
        )}
      </div>
    </div>
  )
...
const Comentarios = ({ videoId, User }) => {
  ...
  const { mutate: enviar, isLoading: enviando } = useMutation(
    ...
    { onSuccess: () => { setTexto(''); queryClient.invalidateQueries(['comentarios', videoId]) } }
  )
    ...
    <div className="mt-8">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-lg font-bold text-gray-700">
          Comentarios
          {comments.length > 0 && <span className="text-sm text-gray-400 font-normal ml-1">· {comments.length}</span>}
        </h3>
      </div>

      <form onSubmit={e => { e.preventDefault(); if (texto.trim()) enviar() }} className="flex gap-2 mb-6">
      ...
      {isLoading ? (
        ...
        <div className="space-y-5">
          {comments.map(c => (
            <ComentarioItem key={c.id} c={c} videoId={videoId} User={User} isAdmin={isAdmin} />
          ))}
        </div>
      )}
    </div>
...
export default function VideoDetail() {
  ...
  const { data: firestoreVideo, isLoading: loadingFirestore } = useQuery(
    ['video-solitario', videoId],
    () => cursoCtrl.getVideo(videoId),
    { enabled: !!videoId, retry: false }
  )

  const { data: todosLosSolitarios = [] } = useQuery(
    ['videos-solitarios', false],
    () => cursoCtrl.getVideosSolitarios(false),
    { staleTime: 1000 * 60 * 5 }
  )

  const relacionados = todosLosSolitarios
    .filter(v => v.id !== videoId)
    .slice(0, 6)

  const video = firestoreVideo && firestoreVideo !== 'Error' ? {
    nombre: firestoreVideo.Titulo,
    descripcion: firestoreVideo.Descripcion,
    url: firestoreVideo.VideoUrl,
    fecha: null,
    esFirestore: true,
  } : null

  const isLoading = loadingFirestore

  return (
    <>
      <Head>
        <title>{video ? `${video.nombre} - CIPM` : 'Video - CIPM'}</title>
        <link rel="icon" href="/logo.svg" />
        <meta name="viewport" content="initial-scale=1.0, width=device-width" />
      </Head>

      <Navbar />
      <Breadcrumb labels={{ [videoId]: video?.nombre }} />
            ...
            <div className="mt-4">
              {isLoading ? (
                ...
                <>
                  ...
                  {video?.fecha && <p className="text-gray-400 text-sm mt-1">{video.fecha}</p>}
                  {video?.descripcion && (
                    <p className="mt-3 text-gray-500 text-sm leading-relaxed">{video.descripcion}</p>
                  )}
                </>
              )}
            </div>

            {/* Comentarios — solo para videos de Firestore con usuario autenticado */}
            {video?.esFirestore && User && videoId && (
                ...
                {relacionados.map(v => (
                  <Link key={v.id} href={`/video/${v.id}`}
                    ...
                    <div className="relative w-20 h-14 rounded-lg overflow-hidden bg-gray-100 shrink-0">
                      <img src={v.ImgUrl?.trim() || '/miniaturavideo.svg'} alt={v.Titulo}
